...
この文書は、人間が判断層を担うための背景、原則集、検品術を収録した参考資料です。Agent が通常の作業で従う短い手順は、親ディレクトリの `SKILL.md` を正本とします。
...
「自分にはAIの出力の質を見分ける能力が足りない」——その自己評価から出発してよい。だが判断力には二種類ある。**生の判断力**(その場で正解を直観する力)と、**手続き的判断力**(正しい問いを立て、検証を仕掛け、誤りを炙り出す手順を回す力)。前者は移植できない。後者は手順として学び、再現性を高められる。本書のパートVIとVIIはそのための道具を示す。そして手続き的判断力は、回すたびに生の判断力を育てる。
...
2. **生成と検証は異なる作業である。** テスト、出典照合、制約確認のように合否基準を外部化できる場合、生成より検証を構造化しやすい。ただし、検証者が同じ盲点を共有する場合や正解基準自体が不明な場合、この利点は成立しない。
...
**これは、判断に迷う場面への先回りの回答である。** 実行層はE6(拮抗)・E7(違和感)でエスカレーションする前に本パートを検索し、該当原則があれば「原則P-IVxxに基づき判断」とログに残して自己解決してよい(E1不可逆・E8非対称リスクは除く——これらは常に人間へ)。人間は裁定に迷ったとき本パートを開く。
...
- **P-IV22 読み手最適化**: コードは書いた後に繰り返し読まれる。賢い一行より凡庸な三行。
- **P-IV23 誤った抽象より重複**: 二箇所の重複はまだ抽象化しない。三箇所目で、本当に同じ変化理由を持つか確認してから抽象化する。誤った抽象を剥がすコストは、短期的な重複の維持コストを上回りうる。
- **P-IV24 動く→正しい→速い の順**: 最適化はボトルネックの計測後のみ。推測による最適化を禁止。
- **P-IV25 状態は敵**: 可変状態を最小化する。多くのバグは「誰かがいつの間にか変えた状態」に住んでいる。
- **P-IV26 失敗の作法**: 開発中は速く大声で失敗させ(fail fast)、本番では静かに劣化させる(graceful degradation)。逆にしない。
- **P-IV27 バグは自分のコードにある**: コンパイラ・標準ライブラリ・成熟したOSSを疑う前に、自分のコードと統合部分の仮説を検証する。
...
- **P-IV37 最後に変わったものを疑う**: 「関係ないはず」の直近の変更も候補から外さず、差分と時系列を確認する。
...
- **P-IV67 分散は無知への最適戦略**: 将来を読めない(ほぼ全員読めない)なら、時間と対象の分散は、予測誤差による集中損失を抑える有力な姿勢。
...
- **T2 自己整合性チェック(発散検知)**: 同じ質問を**新しいコンテキストで**2〜3回聞き、答えを比較する。一致は弱い追加証拠にすぎず、**発散したら外部証拠なしにどの答えも採用しない**。これは弱いモデルに欠けている「確信度の自己申告」を外部から測定する手法であり、低コストで不安定さを発見できる技法。事実確認・計算・重要な推奨では、可能なら外部証拠による確認と組み合わせる。
- **T3 敵対パス**: 成果物を新しいインスタンスに渡し「この内容の欠陥を最大限列挙せよ。良い点は書くな」。同一コンテキストでの自己レビューは同じ盲点を共有しやすい。可能なら新しいコンテキストの別インスタンスを使い、使えなければ自己レビューだと明記する。
- **T4 復唱確認**: 作業開始前に「私の依頼を、あなたの言葉で、成功条件込みで言い直せ」。理解のズレを着手前に発見しやすく、修正コストも低い。
- **T5 文脈供給**: 弱いモデルの失敗には、能力不足だけでなく**文脈不足**が大きく影響する。関連文書・過去の決定・良い出力の実例・使用ライブラリのドキュメントを渡す。人間は重みを変えられないが文脈は自由に変えられる——ここが人間の主戦場。
...
- **T10 状態の外部化**: 長いタスクではモデルの記憶を信用せず、決定文書・進捗ログ・TODOをファイルに書かせて毎セッション読み込ませる。これにより、コンテキスト切れによる状態喪失を減らせる。
...
# パートVI: 検品術 — 専門知識がなくても使える異常検知(人間用)

あなたが判断層を務めるための中核技術。これらは専門知識がなくても、不安定さ、矛盾、説明不足を見つける助けになる。ただし、正しさの証明や専門家の代替にはならない。

## 6.1 五つの基本検査
- **V1 発散検査**: (T2と同じ)新しいコンテキストで同じ質問を2〜3回。答えが揺れたら、外部証拠なしに結論を採用しない。不安定さを専門知識なしで検出できるが、一致は正しさを保証しない。
- **V2 具体性プローブ**: 「その根拠は? 正確な数字は? 出典は?」と掘る。次に別の角度から同じ事実を確認する。細部の漂流は危険信号だが、一貫した回答も正しさを保証しないため、出典そのものを開いて確認する。
- **V3 反論耐性検査**: 主張に対する強い反例や逆の仮説を提示し、根拠に照らして再評価させる。根拠なく即座に翻る場合は、どちらの回答も採用しない。踏みとどまること自体も正しさの証明ではない。
- **V4 平易化梯子**: 「中学生に説明するつもりで言い直せ」。平易版の論理が繋がらなければ追加確認が必要。専門用語の多さを理解の証明として扱わない。
- **V5 桁の常識検査**: 数字が出たら自分で桁だけ暗算する(「日本の人口×これは…桁が合わない」)。精密な検算の前に、桁ズレと単位違いを低コストで発見できる。
...
- **高(金銭・健康・法律・セキュリティ・不可逆・重大な公開影響)**: 適用可能な検査+**AIの外での確認を必須とする**——権威ある指針、適切に選んだ一次証拠、実行テスト、または人間の専門家。この水準では「AIが自信を持っている」は証拠として扱わない。
...
→ 最後に**人間が争点1行に答える**。単独回答の盲点を減らす可能性はあるが、正しさや専門家判断の代替を保証しない。重要な点: 最終決定は必ず人間が行う。評議会は判断を作るのでなく、判断を**可能な形に整える**装置。
...
- 定期的に回せば、手続き的判断力を改善する材料が蓄積する。判断は才能ではなく、記録と照合の複利である。
...
モデルや道具は変わり続ける。それでも、この体系は致命的な誤りを避け、要所であなた自身の判断を増幅するために残る。それで十分に進める。
