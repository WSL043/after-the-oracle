...
図の流れ: 実行(EXECUTE) → 検証(VERIFY) → 人間の判断(JUDGE) → 記録(RECORD)。
...
VS Code/CopilotとWindsurfはプロジェクト内のコピーを検出します。個人用に導入する場合:
...
| 場面 | AIにこう頼む | 良い出力の形 |
|---|---|---|
| リリース計画 | `after-the-oracle Skillを使って。まず計画だけ出して、まだ実行しないで。` | 計画、仮定、リスク、検証手順 |
| AI回答の検証 | `after-the-oracle Skillでこの回答を敵対的に検証して。` | 欠陥候補、不確かな主張、確認方法 |
| 公開判断 | `公開前に判断ブリーフを作って。` | 選択肢、推奨、可逆性、人間への最終質問 |
| 長期プロジェクト継続 | `決定文書を更新して、それに基づいて続けて。` | 現在状態、使った決定、次の行動 |

## License

MITです。[LICENSE](LICENSE) の条件に従って、自由に利用・改変・公開・再配布できます。コンテンツと画像の由来は [NOTICE.md](NOTICE.md) に記録しています。
